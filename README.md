...
__Fig. 1 Model data representation and architecture.__
*Each patient is represented as a temporally ordered sequence of past mammography examinations preceding the index exam. Each element in the sequence consists of the free-text radiologist report encoded as a text embedding, together with the time interval elapsed since the previous examination. Structured clinical features extracted from the electronic health records (EHR) are also available at the moment of prediction. All information is strictly restricted to data available at the time of the index exam, preserving temporal consistency. Report embeddings and inter-examination time intervals are passed through a modular aggregation function that summarizes the full screening history into a single representative vector. This vector is concatenated with the structured clinical features to form the final patient representation, which is used as input to a dense neural network that simultaneously produces calibrated probability estimates for each prediction horizon from one to five years.* 
