    ...
    def _maybe_save_checkpoints(self, epoch: int, fold_ix: int, sel: float):
        ...
        if self.save_epochs:
            ...
            torch.save(
                ...
                self.checkpoint_path.joinpath(self.model_name, fname),
            )
    ...
    def _run_epoch(
        ...
        with ctx:
            for b_idx, batch in enumerate(loader):
                ...
                loss = None
                if self.loss_function_nm != "cross_entropy": # -- it only makes sense if we are trying other cost functions (so far, I am not).
                    if is_train:
                ...
                # -- I will keep this eligibility mask in case I try to build a multi-year model in the future.
                # -- nevertheless, this mask does not bring anything extra for this single-year model.
...
class TrainingSingleYearTFIDF(TrainingBase):
    ...
    def _training_cv_loop(
        ...
        for fold_ix, fold_key in enumerate(fold_keys):
            ...
            # --------------------------------------------
            self._build_model()
            # -- get the current fold split -> IDs for training and for validation
            train_ids = split_dict[fold_key]['train']
            validation_ids = split_dict[fold_key]['validation']
            # -- create the data loader according to the split
                ...
                if not tuning:
                    ...
                    if sel > self.best_sel_metric:
                        ...
                        if self.save_best_epochs:
                            ...
                            torch.save({
                                'epoch': epoch, 'model_state_dict': self.model.state_dict(), 'optimizer_state_dict':self.optimizer.state_dict()
                            }, self.checkpoint_path.joinpath(self.model_name, f'best_model_fold_{fold_ix:.0f}.pt'))
            
            if tuning and fold_ix==0:
                # -- only the first fold will be used for tuning
                break
    ...
    def _cross_validation_loop(
        ...
        fold_keys = [k for k in split_dict.keys() if k != "test"] # 'fold 0', 'fold 1', etc.
        for fold_ix, fold_key in enumerate(fold_keys):
            self._reset_best_metrics_for_fold()
            # -------------------------
            # 1) Split + model + loaders
            # -------------------------
            self._build_model()
            ...
            for epoch in tqdm(range(self.num_epochs)):
                ...
                val_out, _ = self._run_epoch(
                    ...
                    is_train=False,
                    epoch=epoch,
                    opt_step_counter=opt_step_counter,  # not used in val, but kept consistent
    ...
    def train(
        ...
        print("training ...")
        self._training_cv_loop(split_dict, target_year, tuning=tuning, verbose=True)
        #self._cross_validation_loop(split_dict, target_year, tuning=tuning, verbose=True)
